...
%
% version 0.2	last change 17.09 2007
...
if values.up==1
  ...
  disp(['    Mean heading offset based on compass is ',num2str(hoff),' deg'])

  % check tilt sensors
  % rotate by compass offset
  diary off
  hoff2 = fminsearch('checktilt',0,[],...
	[d.rol(2,:);d.pit(2,:);d.rol(1,:);d.pit(1,:)]);
  diary on
  disp(['    Mean heading offset based on tilt is ',num2str(hoff2),' deg'])
  ...
  da = -angle(u1d)+angle(u1u*exp(sqrt(-1)*hoff*pi/180));
  ii = find(da>pi);
  da(ii) = da(ii)-2*pi;
  ii = find(da<-pi);
  da(ii) = da(ii)+2*pi;
  ...
  title([' Heading offset : ',num2str(hoff)])
  xlabel('Heading down')
  grid
...
%  if (p.offsetup2down~=0 & length(d.zd)~=length(d.ru(:,1)) & exist('dr')==1)
  ...
  if (p.rotup2down~=0 & length(d.zd)~=length(d.ru(:,1)))
    ...
    u1uc = exp(-sqrt(-1)*(d.hdg(2,:)-hoff)*pi/180);
