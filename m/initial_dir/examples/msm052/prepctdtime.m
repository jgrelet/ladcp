...
if stn==20 | stn==21 | stn==2	% bad CTD data at the end
  return
end
...
eval(['!cp /mnt/merian_public/public_wiss/MSM-052/CTD/',...
        'uncalibrated/msm052_',int2str0(stn,3),...
...
%
[hdr,data] = hdrload(['data/raw_ctdtime/msm052_',int2str0(stn,3),'_1sec.cnv']);
timctd = data(:,2) + julian([2007,1,0,0,0,0]);
%data = data(:,[5,6,11]); % primary sensors
data = data(:,[5,7,17]); % secondary sensors
...
%
if mversion>=70
  save(['data/ctdtime/ctdtime',int2str0(stn,3)],'timctd','data','-v6')
else
  save(['data/ctdtime/ctdtime',int2str0(stn,3)],'timctd','data')
end
